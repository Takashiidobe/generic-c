...
#define gc_sm_valoff(m)                                                        \
  gc_sm_align_up(sizeof((m)._key), _Alignof(typeof((m)._val)))
// storage stride: value end rounded up to slot alignment, so every slot and
// both of its fields stay aligned
#define gc_sm_stride(m)                                                        \
  gc_sm_align_up(gc_sm_valoff(m) + sizeof((m)._val), gc_sm_align(m))
// raw byte pointer
...
#define gc_sm_cmp(m, sh, k, i)                                                 \
  ((sh)->cmp                                                                   \
       ? (sh)->cmp(&(k), gc_sm_keyptr(m, i), sizeof((m)._key))                 \
...
#define bmap_for(m, it_k, it_v)                                                \
  for (size_t gc_i = 0, gc_n = sm_len(m); gc_i < gc_n; ++gc_i)                 \
    for (int gc_once1 = 1; gc_once1; gc_once1 = 0)                             \
      for (typeof((m)._key) it_k = *gc_sm_keyptr(m, gc_i); gc_once1;           \
           gc_once1 = 0)                                                       \
