...
int main(void) {
  ...
  assert(sm_len(map) == 4);
  assert(bmap_get(map, 2)->value == 200);
  assert(bmap_get(map, 99) == NULL);
  assert(bmap_remove(map, 2));
  assert(!bmap_remove(map, 2));
  assert(sm_len(map) == 3);
  ...
  bmap_for(map, key, value) {
    ...
    assert(value.value == expected_values[index]);
    ++index;
  }
  assert(index == 3);
  bmap_free(map);
  ...
  bmap_for(strings, key, value) {
    assert(strcmp(key, ordered_keys[index]) == 0);
    assert(strcmp(value.value, ordered_values[index]) == 0);
    ++index;
  }
  assert(index == 3);
  bmap_free(strings);
