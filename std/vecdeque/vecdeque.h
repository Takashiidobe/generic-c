...
#define gc_dq_header(d)                                                        \
  ((DQHeader *)(void *)((char *)(d) - offsetof(DQHeader, data)))

// ─── Deque type alias ────────────────────────────────────────────────────
#define Deque(T) T *

// ─── length, capacity, and free ─────────────────────────────────────────
#define dq_len(d) ((d) ? gc_dq_header(d)->len : 0)
#define dq_cap(d) ((d) ? gc_dq_header(d)->cap : 0)
#define dq_free(d)                                                             \
  do {                                                                         \
    if (d) {                                                                   \
      free(gc_dq_header(d));                                                     \
...
static inline void gc_dq_grow(void **arr, size_t elem_size) {
  DQHeader *h = *arr ? gc_dq_header(*arr) : NULL;
...
// ─── random-access get ────────────────────────────────────────────────────
#define dq_get(d, IDX) ((d)[(((IDX) + gc_dq_header(d)->front) % dq_cap(d))])
